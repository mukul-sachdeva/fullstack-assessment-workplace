...
const Step4 = (props) => {
  ...
  useEffect(() => {
    ...
    // SECURITY FIX: Private key should NEVER be stored in localStorage unencrypted
    // Removed localStorage storage of private key for security assessment
    // In production, use proper encryption (e.g., Web Crypto API) or a secure key management service
    // The mnemonic phrase is sufficient for wallet recovery - private key can be derived from it
    setPhrases((wallet.mnemonic?.phrase || wallet.mnemonic).split(' '));
    setAddress(wallet.address);
    // SECURITY: Removed unencrypted private key storage
    // localStorage.setItem('key', wallet.privateKey); // REMOVED FOR SECURITY
  }, []);
